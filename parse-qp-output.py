...
def main():
    ...
    parser.add_argument("output", type=str, help="Output LD-JSON file")
    ...
    if os.path.isfile(args["output"]) and not args["force"]:
        print("output " + args["output"] + " already exists; use -f/--force to overwrite")
        sys.exit(1)
    ...
    with open(args["output"], "a") as of:
        of.truncate()
        for data in parse_file(args["input"]):
