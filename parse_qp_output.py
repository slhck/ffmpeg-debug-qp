...
def generate_logfile(video_filename, force=False, include_macroblock_data=False):
...
def parse_logfile(input_file, compute_averages_only, include_macroblock_data):
    if not input_file.endswith(".debug"):
        print_stderr("You attempted to parse a logfile, but supplied a video file name as input")
        sys.exit(1)
...
def extract_qp_data(
    ...
    try:
        ...
        if not use_logfile:
            logfile = generate_logfile(video, force=force, include_macroblock_data=include_macroblock_data)
        else:
            logfile = video
        ...
        if output_format == "json":
            ...
            with open(output, "w") as of:
                json.dump(SerializableGenerator(
                    parse_logfile(logfile, compute_averages_only, include_macroblock_data)), of)
                sys.exit(0)
        else:
            with open(output, "w") as of:
                if output_format == "csv":
                    of.write(print_data_header() + "\n")
                for data in parse_logfile(logfile, compute_averages_only, include_macroblock_data):
